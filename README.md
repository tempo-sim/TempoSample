...
## TempoROS
[TempoROS](https://github.com/tempo-sim/TempoROS) is a standalone plugin, included in Tempo as an optional submodule. The `TempoROS` and `TempoROSBridge` (which translates and forwards Tempo messages to/from ROS) are disabled by default in the `TempoSample` project. If you use ROS, enabled them by by editing the uproject file.
